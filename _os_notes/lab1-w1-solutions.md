...
The key insight: ticks are coarser than milliseconds, so you divide, not multiply.
...
The loop starts from bit 7 (MSB) down to bit 0 (LSB). `(byte >> i) & 1` extracts each bit.
...
Key: advance `f` an extra step to read the digit, and use `%02x` for zero-padded hex.
...
The key trick is to remember the previous character and re-process it.
...
memdump(char *fmt, char *data)
{
    ...
    for (char *f = fmt; *f; f++) {
        ...
        if (cur == 'r') {
            // parse integer count that follows
            f++;
            int count = 0;
            while (*f >= '0' && *f <= '9') {
                count = count * 10 + (*f - '0');
            ...
            f--;  // back up so outer loop increment is correct
            // repeat prev 'count' times
            char tmp[2] = { prev, 0 };
            for (int i = 0; i < count; i++)
                memdump(tmp, ptr + /* offset depends on prev type */0);
            // simpler: inline the switch with prev:
            // (omitted for brevity — use the same switch logic, just loop)
        } else {
            prev = cur;
            // ... normal switch on cur ...
        }
    }
}
```

A cleaner approach: factor the single-character dispatch into a helper function `process_char(char c, char **ptr)` that returns bytes consumed, then the `'r'` case can call it N times.
...
`atoi("-5")` returns `-5`. When `pause(-5)` is called, the xv6 kernel implementation of `sys_pause` likely casts it to an unsigned integer or checks `<= 0` and returns immediately, so `sleep -5` effectively sleeps for 0 ticks.

**Fix:** Add a guard in `sleep.c`:
