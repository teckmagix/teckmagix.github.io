...
main(int argc, char *argv[])
{
  char *mem_ptr;
  char *region_start;
  int region_size = 8 * 4096;
  int page_idx;

  region_start = sbrk(region_size);

  for(page_idx = 0; page_idx < region_size; page_idx += PGSIZE){
    mem_ptr = region_start + page_idx + 16;
    if(mem_ptr[0] != '\0'){
      printf("%s\n", mem_ptr);
      exit(0);
    }
...
**kernel/proc.c** (in kfork, add after `*(np->trapframe) = *(p->trapframe);`):
```c
