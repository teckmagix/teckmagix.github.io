---
layout: page
title: "Lab Predictions - lab-predictions-d94sv"
lab: lab-predictions-d94sv
description: "Lab predictions - lab-predictions-d94sv"
---
...
main(int argc, char *argv[])
{
  ...
  for(page_idx = 0; page_idx < DATASIZE; page_idx += PGSIZE){
    chunk = heap_start + page_idx;
    if(chunk[16] != '\0'){
      printf("%s\n", chunk + 16);
      exit(0);
    }
...
{
  int monitor_mask;
  argint(0, &monitor_mask);
  struct proc *cur_proc = myproc();
  cur_proc->monitor_mask = (uint32)monitor_mask;
  return 0;
}
```
...
**kernel/syscall.c** (full modified file):
```c
#include "types.h"
#include "param.h"
#include "memlayout.h"
