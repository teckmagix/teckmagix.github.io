---
layout: page
title: "Lab Predictions - lab-predictions-nnpa2"
lab: lab-predictions-nnpa2
description: "Lab predictions - lab-predictions-nnpa2"
---

{% raw %}
```c
#define _GNU_SOURCE
#include <pthread.h>
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>

static int resX = 0;
static int resY = 0;

static long op_counter = 0;
static int finished = 0;

static pthread_mutex_t mtxX = PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t mtxY = PTHREAD_MUTEX_INITIALIZER;

#define LOOP_COUNT 20000

typedef struct {
    int thread_id;
} thread_args_t;

typedef struct {
    int thread_id;
    long ops_done;
    int lastA;
    int lastB;
    long lastTotalOps;
    int lastDone;
} thread_result_t;

static void delay(void)
{
    usleep(1);
}

static thread_result_t* make_result(int id, long ops)
{
    thread_result_t* r = malloc(sizeof(thread_result_t));
    if (!r) { perror("malloc"); exit(1); }

    r->thread_id = id;
    r->ops_done = ops;

    pthread_mutex_lock(&mtxX);
    pthread_mutex_lock(&mtxY);
    r->lastA = resX;
    r->lastB = resY;
    pthread_mutex_unlock(&mtxY);
    pthread_mutex_unlock(&mtxX);

    pthread_mutex_lock(&mtxX);
    r->lastTotalOps = op_counter;
    r->lastDone = finished;
    pthread_mutex_unlock(&mtxX);

    return r;
}

void* worker_AB(void* arg)
{
    thread_args_t* p = (thread_args_t*)arg;
    long ops = 0;

    for (int i = 0; i < LOOP_COUNT && !finished; i++) {

        pthread_mutex_lock(&mtxX);
        pthread_mutex_lock(&mtxY);
        resX++;
        resY++;
        pthread_mutex_unlock(&mtxY);
        pthread_mutex_unlock(&mtxX);

        pthread_mutex_lock(&mtxX);
        op_counter += 2;
        pthread_mutex_unlock(&mtxX);

        ops += 2;

        delay();
    }

    return make_result(p->thread_id, ops);
}

void* worker_BA(void* arg)
{
    thread_args_t* p = (thread_args_t*)arg;
    long ops = 0;

    for (int i = 0; i < LOOP_COUNT && !finished; i++) {

        pthread_mutex_lock(&mtxX);
        pthread_mutex_lock(&mtxY);
        resY += 2;
        resX += 2;
        pthread_mutex_unlock(&mtxY);
        pthread_mutex_unlock(&mtxX);

        pthread_mutex_lock(&mtxX);
        op_counter += 2;
        pthread_mutex_unlock(&mtxX);

        ops += 2;

        delay();
    }

    return make_result(p->thread_id, ops);
}

void* worker_monitor(void* arg)
{
    thread_args_t* p = (thread_args_t*)arg;
    long ops = 0;

    while (!finished) {
        pthread_mutex_lock(&mtxX);
        pthread_mutex_lock(&mtxY);
        int a = resX;
        int b = resY;
        long t = op_counter;
        int d = finished;
        pthread_mutex_unlock(&mtxY);
        pthread_mutex_unlock(&mtxX);

        if ((t % 20000) == 0) {
            printf("[monitor] A=%d B=%d total_ops=%ld done=%d\n", a, b, t, d);
        }

        delay();
    }

    return make_result(p->thread_id, ops);
}

int main(void)
{
    pthread_t t1, t2, t3;

    thread_args_t a1 = { 1 };
    thread_args_t a2 = { 2 };
    thread_args_t a3 = { 3 };

    pthread_create(&t1, NULL, worker_AB, &a1);
    pthread_create(&t2, NULL, worker_BA, &a2);
    pthread_create(&t3, NULL, worker_monitor, &a3);

    thread_result_t *r1 = NULL, *r2 = NULL, *r3 = NULL;

    pthread_join(t1, (void**)&r1);
    pthread_join(t2, (void**)&r2);

    pthread_mutex_lock(&mtxX);
    finished = 1;
    pthread_mutex_unlock(&mtxX);

    pthread_join(t3, (void**)&r3);

    const long expected_total_ops = 4L * LOOP_COUNT;

    printf("\n--- Results (returned to main) ---\n");
    printf("Thread %d ops=%ld lastA=%d lastB=%d lastTotalOps=%ld lastDone=%d\n",
           r1->thread_id, r1->ops_done, r1->lastA, r1->lastB, r1->lastTotalOps, r1->lastDone);
    printf("Thread %d ops=%ld lastA=%d lastB=%d lastTotalOps=%ld lastDone=%d\n",
           r2->thread_id, r2->ops_done, r2->lastA, r2->lastB, r2->lastTotalOps, r2->lastDone);
    printf("Thread %d ops=%ld lastA=%d lastB=%d lastTotalOps=%ld lastDone=%d\n",
           r3->thread_id, r3->ops_done, r3->lastA, r3->lastB, r3->lastTotalOps, r3->lastDone);

    printf("\nFinal Shared: A=%d B=%d total_ops=%ld done=%d\n",
           resX, resY, op_counter, finished);
    printf("Expected: total_ops=%ld done=1\n", expected_total_ops);

    free(r1);
    free(r2);
    free(r3);

    return (op_counter == expected_total_ops && finished == 1) ? 0 : 1;
}
```
{% endraw %}
